---
title: "Elizabeth Commands"
old_id: 4
---
These are the commands supported by Elizabeth, our chat bot.  

### General commands
- `!roll` - Returns a random number from 0 to 100  
- `!roll num` - Returns a random number from 0 to num  
- `!help` - Display help message  
- `!pp [mode]` - Show your current pp. If `mode` is not present, Elizabeth will tell you the amount of PP for your current game mode. If mode is present (it can be `std/taiko/ctb/mania`), Elizabeth will tell you the amount of PP for that gamemode. **This command works only in PMs**
...
Elizabeth has some commands similar to Tillerino. Those commands work only if you send them to Elizabeth through a PM. Remember that PP system has been implemented only on osu!standard and osu!mania. The bot doesn't support beatmaps recommendations at the moment, that functionality wil come later, hopefully.
...
- `!fokabot reconnect` - Reconnect Elizabeth if he's not on online users list anymore  
